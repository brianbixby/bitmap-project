...
const writeFileHelper = module.exports = (data, transformedFilePath, wfCallback) => {
  if(transformedFilePath.substr(transformedFilePath.length - 4) !== '.bmp') throw new Error(`${transformedFilePath} is not a bmp file. Please execute your command again with ${transformedFilePath}.bmp as your new bmp file's name.`);
